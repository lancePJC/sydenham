...
const SkincareProducts = () => {
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6 text-yellow-400">Skincare Products</h1>
            ...
            >
              ...
              <div className="relative w-full h-64 mb-4">
                ...
                <button
                  ...
                  className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-yellow-600 text-white p-2 rounded-full opacity-75 hover:opacity-100"
                >
                  &lt;
                </button>
                <button
                  onClick={() => handleNextImage(item.id, item.images.length)}
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-yellow-600 text-white p-2 rounded-full opacity-75 hover:opacity-100"
                >
                  &gt;
                </button>
                {/* Image Indicators */}
              ...
              <h2 className="text-xl font-semibold text-yellow-600">{item.name}</h2>
              <p className="text-gray-600">{item.desc}</p>
              <p className="text-lg font-bold mt-2 text-yellow-700">KES {item.wholesale}</p>
              <p className="text-sm text-gray-500">Category: {item.category}</p>
              <p className="text-sm text-gray-500">Code: {item.code}</p>
              <button
                onClick={() => handleAddToCart(item)}
                className="mt-4 w-full bg-yellow-500 text-black py-2 rounded hover:bg-yellow-600 transition font-semibold"
              >
                Add to Cart
              </button>
            </div>
