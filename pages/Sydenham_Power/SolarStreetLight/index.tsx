...
export default function SolarStreetLights() {
  const streetLights = products.filter((product) => product.name.includes('Solar Street Light'));
  const { addToCart } = useCart();

  return (
    <>
      <Head>
        <title>Solar Street Lights | Solar Energy Solutions</title>
      </Head>
        ...
        <div className="max-w-6xl mx-auto space-y-16">
          <section className="text-center space-y-4">
            ...
            <p className="text-gray-300 text-lg max-w-2xl mx-auto">
              Energy-efficient street lighting for public and private spaces.
            </p>
          </section>
          <section>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {streetLights.map((product) => (
                <div
                  key={product.id}
