...
export default function SolarFridge() {
  const solarFridges = products.filter((product) => product.name.includes('Solar Fridge'));
  const { addToCart } = useCart();

  return (
    <>
      <Head>
        <title>Solar Fridges | Solar Cooling Solutions</title>
      </Head>
        ...
        <div className="max-w-6xl mx-auto space-y-16">
          <section className="text-center space-y-4">
            ...
            <p className="text-gray-300 text-lg max-w-2xl mx-auto">
              Reliable solar-powered refrigeration for your needs.
            </p>
          </section>
          <section>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {solarFridges.map((product) => (
                <div
                  key={product.id}
