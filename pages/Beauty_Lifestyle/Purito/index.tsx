...
const PuritoPage: React.FC = () => {
  const puritoProducts = skincareItems.filter((item) => item.code.startsWith('PURITO-'));
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6">Purito Products</h1>
      {puritoProducts.length === 0 ? (
        <p className="text-gray-600">No Purito products available.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {puritoProducts.map((item: SkincareItem) => {
