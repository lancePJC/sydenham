...
const CelenesPage: React.FC = () => {
  const celenesProducts = skincareItems.filter((item) => item.code.startsWith('CELENES-'));
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6">Celenes Products</h1>
      {celenesProducts.length === 0 ? (
        <p className="text-gray-600">No Celenes products available.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {celenesProducts.map((item: SkincareItem) => {
