...
const CosrxPage: React.FC = () => {
  const cosrxProducts = skincareItems.filter((item) => item.code.startsWith('COSRX-'));
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6">COSRX Products</h1>
      {cosrxProducts.length === 0 ? (
        <p className="text-gray-600">No COSRX products available.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {cosrxProducts.map((item: SkincareItem) => {
