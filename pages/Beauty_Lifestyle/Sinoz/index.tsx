...
const SinozPage: React.FC = () => {
  const sinozProducts = skincareItems.filter((item) => item.code.startsWith('SINOZ-'));
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6">Sinoz Products</h1>
      {sinozProducts.length === 0 ? (
        <p className="text-gray-600">No Sinoz products available.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {sinozProducts.map((item: SkincareItem) => {
